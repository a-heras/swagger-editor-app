...
function DetailRow({ label, value }: DetailRowProps) {
    return (
        <div className="rounded-lg border border-cyan-300/10 bg-black/30 p-4">
...
export function HistoryDetail({ item }: HistoryDetailProps) {
    return (
        <div className="flex flex-col gap-6">
            <Link
                href="/history"
                className="w-fit text-sm font-semibold text-cyan-100/70 transition hover:text-cyan-300"
            >
                ← Back to history
            </Link>

            <article className="rounded-2xl border border-cyan-300/20 bg-slate-950/70 p-6 shadow-[0_0_35px_rgba(34,211,238,0.12)] backdrop-blur-xl">
                <div className="flex flex-wrap items-center gap-3">
                    <span className="rounded-md bg-cyan-300 px-2 py-1 text-xs font-black uppercase text-slate-950">
                        {item.method}
                    </span>
                    <h2 className="font-mono text-lg font-semibold text-cyan-100">
                        {item.url}
                    </h2>
                </div>
